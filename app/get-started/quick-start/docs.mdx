...
export default ({ children }) => <DocLayout current="quick-start" previous={{href: '/get-started/introduction', label: 'General Introduction'}} next={{href: '/react-native-guide/introduction', label: 'React Native Introduction'}}>{children}</DocLayout>