...
export default ({ children }) => <DocLayout current="quickstart" previous={{href: '/react-native-guide/introduction', label: 'React Native Introduction'}} next={{href: '/react-native-guide/integration/how-it-works', label: 'How it Works'}}>{children}</DocLayout>
