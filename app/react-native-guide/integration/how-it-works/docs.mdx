...
export default ({ children }) => <DocLayout current="how-it-works" previous={{href: '/react-native-guide/integration/quickstart', label: 'Quickstart'}} next={{href: '/react-native-guide/mobile-features/overview', label: 'Overview'}}>{children}</DocLayout>