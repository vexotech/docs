...
export default ({ children }) => <DocLayout current="rn-integration" previous={{href: '/react-native-guide/introduction', label: 'Introduction'}} next={{href: '/react-native-guide/mobile-features/overview', label: 'Overview'}}>{children}</DocLayout>