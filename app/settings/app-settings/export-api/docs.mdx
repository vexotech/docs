...
export default ({ children }) => <DocLayout current="export-api" previous={{href: '/settings/app-settings/webhooks', label: 'Webhooks'}} next={{href: '/settings/app-settings/team', label: 'Team'}}>{children}</DocLayout>